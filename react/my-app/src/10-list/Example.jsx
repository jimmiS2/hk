...
// 3. c 사용하여, tasks 배열을 2개 생성후 사용한다.
...
export function LoadPost() {
  ...
  function AddPost() {
    if (newPost.id && newPost.userId && newPost.title && newPost.body) {
      setNewdata([...newdata, newPost]);
      setNewPost({ id: "", userId: "", title: "", body: "" });
    }
    console.log(posts);
  }
  ...
  return (
    <div>
      ...
      <ul>
        {posts
          .slice(0, 10 + posts.filter((p) => p.id > 100).length)
          .map((post) => (
            <li key={post.id}>
              <p>ID: {post.id}</p>
