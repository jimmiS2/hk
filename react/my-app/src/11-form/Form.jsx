...
export function ControledForm() {
  ...
  const [textareaValue, setTextareaVaule] = useState("");
  const [selectValue, setSelectVaule] = useState("option1");

  const handleInputChange = (e) => {
    setInputValue(e.target.value);
  };
  const handleTextChange = (e) => {
    setTextareaVaule(e.target.value);
  };
  const handleSlectChange = (e) => {
    setSelectVaule(e.target.value);
  };

  const handleSubmit = (e) => {
    e.preventDefalut(); // submit 통신 방지
    alert(
  ...
  return (
    <form action="#" onSubmit={handleSubmit}>
      <div>
        ...
        <label> 설명 : </label>
        <textarea value={textareaValue} onChange={handleTextChange}></textarea>
        <div>
          <label> 선택 : </label>
          <select value={selectValue} onChange={handleSlectChange}>
...
// 비제어 컴포넌트
// ref를 사용하여 입력값을 직접 DOM에서 읽는 방식

export function UncontrolledForm() {
  const inputRef = useRef.apply(null);
  const textareaRef = useRef.apply(null);
  const selectRef = useRef.apply(null);

  const handleSubmit = (e) => {
    //e.preventDefalut();
    alert(
      `입력값: ${inputRef.current.value} 텍스트: ${textareaRef.current.value} 선택값: ${selectRef.current.value}`
    );
  };
