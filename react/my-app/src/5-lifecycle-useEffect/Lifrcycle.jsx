...
export class LifecycleClass extends Component {
  ...
  componentWillUnmount() {
    console.log("컴포넌트가 언마운트 됨");
  }
  // 상태나 props가 변경시 호출됨(업데이트)
  componentDidUpdate() {
    console.log(`컴포넌트가 업데이트 됨 ${this.state.count}`);
  }
...
export function LifecycleFunc() {
  ...
  //마운트 & 언마운트
  useEffect(() => {
    console.log("컴퍼넌트가 마운트 됨.");
    return () => {
      console.log("컴퍼넌트가 언마운트 됨");
    };
  }, []); // 빈 배열을 넣으면 (언)마운트시 한번만 호출
